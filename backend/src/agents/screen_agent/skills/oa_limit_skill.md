...
1. **最终筛选结果不超过 10 篇**：`screened_paper_ids` 列表长度 ≤ 10。
...
2. **优先选择 Open Access（OA）文献**：
   ...
   - demo 版本每次运行最多下载 1 篇 PDF；正式版不受此限制
...
- **download_papers 步骤**：对每篇 screened_paper_ids 逐篇尝试下载，失败则继续，不终止
...
```json
{
  ...
  "screened_count": 10,
  "excluded": [...],
  "screen_summary": "共筛选 N 篇候选，相关 M 篇（OA 优先，取前 10 篇）"
}
```
