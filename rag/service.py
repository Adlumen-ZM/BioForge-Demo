...
依赖前提
--------
运行本文件前，PepClaw 的以下模块必须可被 Python import：
    rag_pipeline.parser.ragflow_parser      (RAGFlowParser)
    rag_pipeline.retrieval.bge_hybrid_retriever (BGEHybridRetriever)
    rag_pipeline.core.orchestrator          (PipelineOrchestrator)

推荐做法：将 PepClaw 仓库根目录加入 PYTHONPATH，或 pip install -e /path/to/PepClaw
"""
...
class BioPaperRAGService:
    ...
    def _get_parser(self):
        ...
        if self._parser is None:
            # 导入 PepClaw 内部模块（需确保 PepClaw 在 PYTHONPATH 中）
            from rag_pipeline.parser.ragflow_parser import RAGFlowParser  # noqa: PLC0415
    ...
    def _get_orchestrator(self):
        ...
        if self._orchestrator is None:
            from rag_pipeline.core.orchestrator import PipelineOrchestrator  # noqa: PLC0415
            self._orchestrator = PipelineOrchestrator(
                ragflow_base_url=self._ragflow_base_url,
                ragflow_api_key=self._ragflow_api_key,
                llm_api_key=self._llm_api_key,
                llm_base_url=self._llm_base_url,
                llm_model=self._llm_model,
                scout_model=self._scout_model,
                bge_model_dir=self._bge_model_dir,
                bge_use_fp16=self._bge_use_fp16,
    ...
    def retrieve_evidence(
        ...
        try:
            ...
            # 延迟加载 BGE 检索器（首次调用会下载/加载模型，耗时约 30-60s）
            if self._retriever is None:
                from rag_pipeline.retrieval.bge_hybrid_retriever import (  # noqa: PLC0415
                    BGEHybridRetriever,
                )
            ...
            # 重建索引并检索（BGE 在内存中对 chunks 建立临时索引）
            hits = self._retriever.retrieve(chunks=chunks, query=query, top_k=top_k)

            evidence = [
                {"chunk_id": h.get("chunk_id", f"chunk_{i}"), "text": h["text"]}
